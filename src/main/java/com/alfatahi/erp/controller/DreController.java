...
public class DreController {
    ...
    public DreController(AccountsReceivableRepository recRepo,
                         AccountsPayableRepository payRepo) {
    ...
    public String generateDre(
        ...
        if ("annual".equals(type)) {
            ...
            for (int m = 1; m <= 12; m++) {
                ...
                coluna.addReceita(nvl(recRepo.sumReceivedByMonthAndYear(inicio, fim)));
                coluna.addCmv(nvl(payRepo.sumCmvByMonthAndYear(inicio, fim)));
                coluna.addDespesa(nvl(payRepo.sumDespesasFixasByMonthAndYear(inicio, fim)));
                coluna.addDespesaFinanceira(nvl(payRepo.sumDespesasFinanceirasByMonthAndYear(inicio, fim)));
            }
            colunas.add(coluna);
        } else {
